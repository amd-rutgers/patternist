...
p5.prototype.EMOJI_IMAGES_PATH = 'assets/emoji/';
p5.prototype.registerPreloadMethod('loadEmoji', p5.prototype);
...
p5.prototype.loadEmoji = function(symbol, success, fail) {
  return this.loadImage(this.getEmojiUrl(symbol), success, fail);
}

/**
 * Get the image url for an emoji
 */
p5.prototype.getEmojiUrl = function(symbol) {
  var code, path;
  var emojis = this.EMOJI_MAP;
  
  if(!emojis[symbol]) {
    console.error('Emoji '+symbol+' not found');
    return;    
  }

  code = emojis[symbol][2];

  if(code.length > 6) {
    code = code.slice(0, 5) + '-' + code.slice(5);
  }

  path = this.EMOJI_IMAGES_PATH + code + '.png';

  return path;
}

/**
 * Draw the emoji using same arguments as `image`
 */
p5.prototype.drawEmoji = function() {
  this.image.apply(this, arguments);
}


// nabbed this from the emoji npm module
// https://github.com/node-modules/emoji
p5.prototype.EMOJI_MAP = {
  // missing
